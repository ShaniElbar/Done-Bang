...
@section('main-content')
    <div class="w-full h-auto">
        <section>
            <div class="buttonTeam w-full flex items-center" id="buttonShow">
                <p class="w-[80%] text-[26px] font-[700]">{{ $categoryDetail->name }}</p>
                <div class="flex items-center cursor-pointer">
                    ...
                    <p class="ml-[5px] text-[14px] gray-500">Add team</p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('teams.invite') }}">
                ...
                <div
                    class="AddTeamShow hidden absolute ml-[80px] w-[90%] max-w-md h-[50vh bg-white rounded-2xl shadow-lg z-[1000]">

                    <div class="w-full h-12 flex items-center border-b px-4 border-gray-200">
                        <p class="text-base font-semibold text-gray-800">Add Teams</p>
                    </div>


                    <div class="w-full h-[33vh] px-4 py-4">
                        <label class="block font-medium text-sm text-gray-700 mb-1 mt-2">Email</label>
                        <input name="email" type="email" required
                            class="w-full text-sm border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#335166] focus:border-[#335166] transition duration-200">

                        <input type="hidden" name="category_id" value="{{ $categoryDetail->id }}">
                        <input type="hidden" name="role" value="guest">

                        <div class="mt-[10px] h-[16vh]" style="overflow-x: auto">
                            <p class="text-[14px] text-gray-500">participant</p>



                            {{-- <div
                                    class="w-full rounded-[5px] mt-[5px] h-[35px] border-[1px] border-gray-300 flex items-center">

                                    <img class="w-[30px] h-[30px] rounded rounded-[50%]"
                                        src="{{ asset('storage/' . $teams->user->photo) }}" alt="">

                                    <p class="text-[14px] text-gray-500 ml-[10px] w-[72%]">{{ $teams->user->name }}
                                    </p>

                                    <p class="text-[14px] text-gray-500">{{ $teams->role }}</p>
                                </div> --}}
                            <div
                                class="w-full rounded-[5px] mt-[5px] h-[35px] border-[1px] border-gray-300 flex items-center">

                                <img class="w-[30px] h-[30px] rounded rounded-[50%]"
                                    src="{{ asset('storage/' . $categoryDetail->user->photo) }}" alt="">

                                <p class="text-[14px] text-gray-500 ml-[10px] w-[72%]">{{ $categoryDetail->user->name }}</p>

                                <p class="text-[14px] text-gray-500">Owner</p>
                            </div>
                            ...
                            @foreach ($teamMembers  as $team)
                                <div
                                    ...
                                    <p class="text-[14px] text-gray-500 ml-[10px] w-[72%]">{{ $team->name }}
                                    </p>

                                    <p class="text-[14px] text-gray-500">{{ $team->role }}</p>
                                </div>
                            @endforeach
                    ...
                    <div class="h-12 flex justify-end items-center border-t px-4 border-gray-200">
                        @if ($categoryDetail->user_id === $user->id)
                            <button type="submit"
                                class="bg-[#335166] hover:bg-[#273c4a] text-white text-sm font-medium px-4 py-2 rounded-md transition duration-200">
                                Submit
                            </button>
                        @else
                            <button type="button"
                                class="bg-[#335166] cursor-not-allowed text-white text-sm font-medium px-4 py-2 rounded-md opacity-50">
                                Submit
                            </button>
                        @endif
                    </div>
        ...
        <script>
            ...
            const addTeamShow = document.querySelector('.AddTeamShow');


            buttonShow.addEventListener('click', (e) => {
                e.stopPropagation();
                addTeamShow.classList.toggle('hidden');
            });

            document.addEventListener('click', (e) => {
                if (!addTeamShow.contains(e.target) && !buttonShow.contains(e.target)) {
                    addTeamShow.classList.add('hidden');
                }
            });


            addTeamShow.addEventListener('click', (e) => {
                e.stopPropagation();
            });
        </script>
                ...
                @foreach ($list as $item)
                    <div class="draggable-item" data-id="{{ $item->id }}" draggable="true">
                        <div class="w-full p-[10px] h-auto flex cursor-pointer">
                            <div class="w-auto h-[20px] hover:bg-[#b8b8b8] flex items-center justify-center rounded-[5px]">
                                        ...
                                        <div class="w-full h-auto py-[10px] px-[10px]  bg-white mt-[1px]">
                                            ...
                                            <div onclick="event.preventDefault(); document.getElementById('complete-form-{{ $item->id }}').submit();"
                                                ...
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    fill="none" viewBox="0 0 24 24" stroke="#929292" stroke-width="2"
